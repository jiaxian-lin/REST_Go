...
export PATH=$JAVA_HOME/bin:$PATH
cd ../../jdk11/cwa-verification && mvn clean install -DskipTests && mvn dependency:build-classpath -Dmdep.outputFile=cp.txt
cd ../market && mvn clean install -DskipTests && mvn dependency:build-classpath -Dmdep.outputFile=cp.txt
